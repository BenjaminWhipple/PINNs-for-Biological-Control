...
def f(U,I,V,t,net):
    ...
    pde = S_t + e1*I - (S_I**2)*(1/4)*(I**2) + (S_U)*(-b*V*U) + (S_I)*(b*V*U-a*I) + (S_V)*(k*I-l*V)
    return pde
...
net_bc_out = net(pt_U_bc, pt_I_bc, pt_V_bc, pt_t_bc)
mse_u = mse_cost_function(net_bc_out, pt_u_bc)
f_out = f(pt_U_collocation, pt_I_collocation, pt_V_collocation, pt_t_collocation, net) # output of f(x,t)
mse_f = mse_cost_function(f_out, pt_all_zeros)
loss = mse_f+mse_u
print(loss.data)

torch.save(net,"TargetCellModel_Deterministic_Control_Value.pt")
